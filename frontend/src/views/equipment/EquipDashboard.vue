    ...
    <div class="card p16">
      ...
      <template v-else>
          ...
          <div class="card p16">
            ...
            <ul class="legend">
              <li v-for="p in ticketParts" :key="p.label">
                ...
                {{ p.label }}: {{ fmt(p.value) }} ({{ todayTotal ? ((p.value/todayTotal)*100).toFixed(1) : 0 }}%)
              </li>
            </ul>
          </div>
        </div>
        ...
        <div class="card p16">
          ...
          <p class="note">
            Currently, there are <b>{{ fmt(totalDevices) }}</b> devices, of which <b>{{ fmt(overview.inUse) }}</b> are in use (utilization rate
            <b>{{ (utilizationRate * 100).toFixed(1) }}%</b>). <b>{{ fmt(overview.underRepair) }}</b> devices are under repair,
            and <b>{{ fmt(overview.pendingScrap) }}</b> devices are pending scrap. Today, there are a total of <b>{{ fmt(todayTotal) }}</b> tickets,
            with <b>{{ fmt(today.inProgress) }}</b> in progress, <b>{{ fmt(today.pendingResponse) }}</b> awaiting response,
...
.btn { padding:8px 12px; border-radius: 8px; border: 1px solid #e5e7eb; background:#111827; color:#fff; font-size: 13px; }
.btn.ghost { background:#fff; color:#111827; }
.btn:disabled { opacity: .6; cursor:not-allowed; }
.grid { display:grid; gap: 16px; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); }

/* kpis */
.kpis { display:grid; grid-template-columns: repeat(auto-fit, minmax(220px,1fr)); gap:16px; margin-top:16px; }
...
.kpi-desc { font-size:12px; color:#6B7280; margin-top: 6px;}
...
/* viz */
.viz { display:grid; grid-template-columns: repeat(auto-fit, minmax(280px,1fr)); gap:16px; margin-top:16px; }
.subtitle { font-weight:600; margin-bottom:8px; }
.legend { display:flex; flex-direction: column; gap:6px; margin-top:10px; font-size:13px; color:#374151; }
...
.donut-text { font-size: 14px; fill:#111827; }

/* table */
.table { width:100%; border-collapse: collapse; font-size: 14px; }
.table th, .table td { padding:8px 10px; border-bottom:1px solid #f0f0f0; }
.table th { text-align:left; color:#6B7280; font-weight:500; }
.table .right { text-align:right; }

/* states */
.skeleton { height: 84px; background: linear-gradient(90deg,#f3f4f6 25%,#eceff3 37%,#f3f4f6 63%); border-radius:12px; animation: shimmer 1.2s infinite; }
@keyframes shimmer { 0% { background-position: -200px 0; } 100% { background-position: calc(200px + 100%) 0; } }
.error { padding: 16px; color:#B91C1C; }

/* misc */
.note { color:#374151; line-height:1.6; }
.timestamp { color:#6B7280; font-size:12px; margin-top:6px; }
</style>
