    ...
    <div class="card" style="padding:16px;">
        ...
        <div class="ui-toolbar" style="margin-top:16px; display:flex; flex-wrap:wrap; gap:12px;">
            ...
            <div style="min-width:200px;">
                <label>Type</label>
                <MultiSelect v-model="filters.types" :options="types.map(t => ({ value: t, label: t }))"
                             placeholder="All types"/>
            </div>
            <div style="min-width:200px;">
                <label>Status</label>
                <MultiSelect v-model="filters.statuses" :options="statuses.map(s => ({ value: s, label: s }))"
        ...
        <!-- Device cards -->
        <div class="cards"
             style="margin-top:16px; display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:12px;">
            <div v-for="d in filtered" :key="d.equipmentId" class="device-card card" @click="openDetail(d)"
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-info">
                        <div style="display:grid; gap:8px;">
                            <div><b>Type:</b> {{ detail.device.equipmentTypeName || detail.device.equipmentTypeId }}
                            </div>
                            ...
                            <div><b>Manual:</b>
                                <a v-if="detail.device.userManualPath"
                                   :href="`http://localhost:8080${detail.device.userManualPath}`"
                                   target="_blank">open</a>
                                <span v-else>-</span>
                            </div>
                            <div><b>Warranty:</b>
                                <a v-if="detail.device.warrantyCertificatePath"
                                   :href="`http://localhost:8080${detail.device.warrantyCertificatePath}`"
                                   target="_blank">open</a>
                                <span v-else>-</span>
                            </div>
                        </div>
