<template>
    <div class="card" style="padding:16px;">
        ...
        <div class="ui-toolbar" style="margin-top:16px;">
            <div>
                <label>Status</label>
                <MultiSelect v-model="filters.statuses"
                             :options="statuses.map(s => ({ value: s, label: statusLabels[s] }))"
                             placeholder="All status"/>
            </div>
            <div>
                <label>Department</label>
                <MultiSelect v-model="filters.departments" :options="departmentList.map(d => ({ value: d, label: d }))"
        ...
        <div class="table-wrapper" style="margin-top:16px; overflow:auto;">
            <table class="ui-table" style="table-layout:auto; width:100%;">
                ...
                <tr v-for="t in filtered" :key="t.ticketId">
                    ...
                    <td><input class="input" v-model.number="t.cost" type="number" min="0" step="1"
                               style="width:120px;"/></td>
                    <td><input class="input" v-model="t.result" placeholder="Result/Acceptance"/></td>
                    <td>
                        <select class="input" v-model="t.status">
                            <option v-for="s in allowedStatusOptions(t.status)" :key="s" :value="s">{{
                                    statusLabels[s]
                                }}
                            </option>
                        </select>
                    ...
                    <td style="white-space:nowrap;">
                        ...
                        <button class="btn" style="margin-left:8px;" @click="advanceStatus(t)"
                                :disabled="t.status==='completed' || t.status==='rejected'">Advance
                        </button>
                    </td>
                </tr>
...
function allowedStatusOptions(current) {
    switch (current) {
        ...
        case 'in-progress':
            return ['in-progress', 'under-acceptance']
        case 'under-acceptance':
            return ['under-acceptance', 'completed', 'rejected']
        default:
            return [current]
    }
}
