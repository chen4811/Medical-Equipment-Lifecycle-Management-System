    ...
    <div class="card" style="padding:16px;">
        ...
        <div class="ui-toolbar" style="margin-top:16px;">
            ...
            <div>
                ...
                <MultiSelect v-model="filters.roleIds" :options="roles.map(r => ({ value: r.id, label: r.name }))"
                             placeholder="All roles"/>
            </div>
            <div>
                <label>Department</label>
                <MultiSelect v-model="filters.departmentIds"
        ...
        <div class="table-wrapper" style="margin-top:16px; overflow:auto;">
            <table class="ui-table" style="width:100%; max-width: 100%; table-layout:auto;">
                ...
                <tr v-else-if="pagedUsers.length === 0">
                    <td colspan="6">
                        <EmptyState title="No users" hint="Try adjusting filters or add a new user."/>
                    </td>
                </tr>
                ...
                <div class="ui-form-grid">
                    ...
                    <div v-if="modal.mode==='create' || modal.mode==='edit'">
                        ...
                        <div style="display:flex; gap:8px; align-items:center;">
                            <input class="input" v-model="modal.form.password"
                                   :type="showPassword ? 'text' : 'password'" placeholder="Enter password"/>
                            <button class="btn btn-blue" style="width:auto;" @click="showPassword=!showPassword">
...
async function save() {
    ...
    if (!p.username || !p.username.trim()) {
        return showDialog('Name is required')
    }
    if (modal.mode === 'create' && (!p.password || !p.password.trim())) {
        return showDialog('Password is required')
    }
    ...
    if (modal.mode === 'create') {
        const resp = await fetch('/req/admin/user', {
            ...
            body: JSON.stringify({
                name: p.username,
                password: p.password ? await sha256Hex(p.password) : '',
                role: denormalizeRole(p.roleId),
        ...
        if (json.code !== '000') {
            return showDialog(json.message || 'Failed to add user')
        }
    } else {
        const resp = await fetch('/req/admin/user', {
            method: 'PUT',
        ...
        const json = await resp.json().catch(() => ({code: 'ERR'}))
        if (json.code !== '000') {
            return showDialog(json.message || 'Failed to update user')
        }
    }
    await refresh()
    closeModal()
