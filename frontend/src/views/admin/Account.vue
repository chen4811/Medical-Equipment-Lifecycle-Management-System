        ...
        <div class="ui-form-grid">
          <div>
            ...
            <div style="display:flex; gap:8px; align-items:center;">
              <input class="input" :type="show.cur ? 'text' : 'password'" v-model="pwd.current" placeholder="Current password" />
              <button class="btn" style="width:auto;" @click="show.cur=!show.cur">{{ show.cur ? 'Hide' : 'Show' }}</button>
            </div>
          </div>
          <div>
            <label>New Password</label>
            <div style="display:flex; gap:8px; align-items:center;">
              <input class="input" :type="show.new ? 'text' : 'password'" v-model="pwd.next" placeholder="New password" />
              <button class="btn" style="width:auto;" @click="show.new=!show.new">{{ show.new ? 'Hide' : 'Show' }}</button>
            </div>
          </div>
          <div>
            <label>Confirm New Password</label>
            <div style="display:flex; gap:8px; align-items:center;">
              <input class="input" :type="show.confirm ? 'text' : 'password'" v-model="pwd.confirm" placeholder="Confirm new password" />
              <button class="btn" style="width:auto;" @click="show.confirm=!show.confirm">{{ show.confirm ? 'Hide' : 'Show' }}</button>
            </div>
          </div>
        </div>
...
async function saveProfile() {
  try {
    ...
    const resp = await fetch('/req/admin/user', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ account_id: String(accountId), name: profile.name, password: '', role: '', department_id: '', email: profile.email }) })
