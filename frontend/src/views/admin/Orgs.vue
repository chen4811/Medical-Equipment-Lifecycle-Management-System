        ...
        <div class="table-wrapper" style="margin-top:16px; overflow:auto;">
            <table class="ui-table" style="width:100%; max-width:100%; table-layout:auto;">
                ...
                <tr v-else-if="filtered.length===0">
                    <td colspan="3">
                        <EmptyState title="No departments" hint="Create a department to get started."/>
                    </td>
                </tr>
...
async function save() {
    ...
    if (!payload.name || !payload.name.trim()) {
        return showDialog('Department name is required')
    }
    const operatorId = localStorage.getItem('account_id') || '0'
    if (modal.mode === 'create') {
        const resp = await fetch('/req/admin/newDepartment', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({department_name: payload.name, operator_id: operatorId})
        })
        const json = await resp.json().catch(() => ({code: 'ERR'}))
        if (json.code !== '000') {
            return showDialog(json.message || 'Failed to add department')
        }
    } else {
        const resp = await fetch('/req/admin/changeDepartmentName', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({department_id: payload.id, department_name: payload.name, operator_id: operatorId})
        })
        const json = await resp.json().catch(() => ({code: 'ERR'}))
        if (json.code !== '000') {
            return showDialog(json.message || 'Failed to update department')
        }
    }
    await refresh()
    closeModal()
