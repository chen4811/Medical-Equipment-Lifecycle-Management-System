        ...
        <div class="table-wrapper" style="margin-top:16px; overflow:auto;">
            <table class="ui-table" style="width:100%; max-width:100%; table-layout:auto;">
                ...
                <tr v-else-if="paged.length===0">
                    <td colspan="3">
                        <EmptyState title="No logs" hint="Try changing date range or keyword."/>
                    </td>
                </tr>
...
function exportCsv() {
    try {
        fetch('/req/admin/log', {
            ...
            body: JSON.stringify({
                log_action: 'Export CSV (logs)',
                log_user_id: localStorage.getItem('account_id') || '0'
            })
        })
    } catch {
    }
...
async function refreshLogs() {
    try {
        loading.value = true;
        const r = await fetch('/req/admin/logs');
        const j = await r.json();
        if (j.code === '000') state.logs = j.data || []
    } finally {
        loading.value = false
    }
}
