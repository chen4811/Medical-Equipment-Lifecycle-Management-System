...
function openCreate() {
    modal.open = true;
    modal.form = {deviceId: (deviceOptions.value[0]?.value || ''), remark: ''}
}
...
async function save() {
    try {
        ...
        await axios.post('/req/dept/usage/logs', {
            ...
            targetEquipmentId: modal.form.deviceId,
...
;(async () => {
    try {
        const r = await axios.get('/req/devices');
        const arr = Array.isArray(r.data) ? r.data : [];
        deviceOptions.value = arr.map(x => ({
            value: x.equipmentId,
            label: `${x.equipmentId} - ${x.equipmentTypeName || ''}`
        }));
        if (!modal.form.deviceId && deviceOptions.value[0]) modal.form.deviceId = deviceOptions.value[0].value;
        fetchLogs()
    } catch {
    }
})()
